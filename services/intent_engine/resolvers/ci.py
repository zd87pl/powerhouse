...
class CIPipelineResolver(Resolver):
    ...
    def get_actual_state(self, intent: Any) -> Dict[str, Any]:
        return {"exists": False, "runner": "github_actions", "lint": True, "typecheck": True,
                "test": True, "secrets_scan": True,
    ...
    def apply(self, intent: Any, drifts: List[Drift]) -> ReconciliationResult:
        return ReconciliationResult(resource_key=self.resource_key, status=ResourceStatus.SKIPPED,
                                     action_taken="stub — CI scaffolding uses project-scaffold skill", drifts_found=drifts)

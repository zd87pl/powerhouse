...
class GitHubResolver(Resolver):
    ...
    def get_actual_state(self, intent: Any) -> Dict[str, Any]:
        ...
        try:
            resp = requests.get(f"{self.api_url}/repos/{intent.project}", headers=self._headers(), timeout=10)
            if resp.status_code == 200:
                data = resp.json()
                return {"exists": True, "project": data.get("name", ""), "description": data.get("description", ""),
                        "private": data.get("private", False), "html_url": data.get("html_url", "")}
    ...
    def apply(self, intent: Any, drifts: List[Drift]) -> ReconciliationResult:
        return ReconciliationResult(resource_key=self.resource_key, status=ResourceStatus.CREATING,
                                     action_taken="stub — real creation requires GITHUB_TOKEN", drifts_found=drifts)

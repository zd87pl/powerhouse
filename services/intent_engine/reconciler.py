...
def reconcile(intent: IntentFile, dry_run: bool = False) -> List[ReconciliationResult]:
    ...
    for resource_key in intent.resource_keys:
        ...
        if resolver is None:
            results.append(ReconciliationResult(resource_key=resource_key, status=ResourceStatus.SKIPPED,
        ...
        try:
            ...
            results.append(ReconciliationResult(resource_key=resource_key, status=ResourceStatus.ERROR,
                                                 error_message=f"failed to get actual state: {e}",
                                                 duration_ms=(time.monotonic() - t0) * 1000))
            continue
        drifts = resolver.diff(_declared_state(intent, resource_key), actual)
        if not drifts:
            results.append(ReconciliationResult(resource_key=resource_key, status=ResourceStatus.EXISTS,
                                                 action_taken="no drift detected",
                                                 duration_ms=(time.monotonic() - t0) * 1000))
            continue
        if dry_run:
            results.append(ReconciliationResult(resource_key=resource_key, status=ResourceStatus.DRIFTED,
        ...
        try:
            ...
            results.append(ReconciliationResult(resource_key=resource_key, status=ResourceStatus.ERROR,
                                                 error_message=f"apply failed: {e}", drifts_found=drifts,
                                                 duration_ms=(time.monotonic() - t0) * 1000))
    return results
...
def reconcile_summary(results: List[ReconciliationResult]) -> Dict[str, Any]:
    ...
    return {"total_resources": len(results), "by_status": by_status, "total_drifts": total_drifts,
            "errors": errors, "healthy": len(errors) == 0}


def _declared_state(intent: IntentFile, resource_key: str) -> Dict[str, Any]:
    if resource_key == "github_repo":
        return {"exists": True, "project": intent.project, "description": intent.description}
    elif resource_key.startswith("deploy_"):
        provider = resource_key.replace("deploy_", "")
        return {"exists": True, "provider": provider, "region": intent.deploy.region,
        ...
        return {"exists": True, "runner": intent.ci.runner.value, "lint": intent.ci.lint,
                "typecheck": intent.ci.typecheck, "test": intent.ci.test, "secrets_scan": intent.ci.secrets_scan}
    return {}

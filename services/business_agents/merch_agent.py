...
class MerchAgent:
    ...
    def __init__(
        ...
        self.margin_thresholds = (
            margin_thresholds or self.DEFAULT_MARGIN_THRESHOLDS.copy()
        )
        self.restock_thresholds = (
            restock_thresholds or self.DEFAULT_RESTOCK_THRESHOLDS.copy()
        )
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "MerchAgent started. Monitoring %d categories.", len(self.margin_thresholds)
        )
    ...
    async def check_margins(self, products: list[dict]) -> list[ProductAlert]:
        ...
        for p in products:
            ...
            threshold = self.margin_thresholds.get(
                category, self.margin_thresholds["default"]
            )
        ...
        if alerts:
            await self._bus.emit_nowait(
                Event(
                    ...
                    payload={
                        "alerts": len(alerts),
                        "products": [a.product_id for a in alerts],
                    },
                    source="merch_agent",
    ...
    async def check_restock(self, inventory: list[dict]) -> list[ProductAlert]:
        ...
        for item in inventory:
            ...
            threshold = self.restock_thresholds.get(
                category, self.restock_thresholds["default"]
            )
    ...
    async def _handle_task(
        ...
        if (
            "restock" in task_lower
            or "stock" in task_lower
            or "inventory" in task_lower
        ):
